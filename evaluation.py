...
def count_component1(sql):
    ...
    count += len([cond_unit for cond_unit in cond_units if cond_unit[1] == ('not', 'between', '=', '>', '<', '>=', '<=', '!=', 'in', 'like', 'is', 'exists').index('like')])
...
def has_agg(unit):
    return unit[0] != ('none', 'max', 'min', 'count', 'sum', 'avg').index('none')
...
def parse(predict, db_name, db_dir):
...
def evaluate(gold, predict, db_name, db_dir, kmaps):
    evaluator = Evaluator()
    ...
    score = np.mean([v['f1'] for v in partial_scores.values()])
    return score
...
if __name__ == '__main__':
    kmaps = build_foreign_key_map_from_json('dataset/spider_tables.json')

    with open('/local2/jiang719/databases/spider/gt.sql', 'r') as f:
        gt_list = f.readlines()
    with open('/local2/jiang719/databases/spider/pred.sql', 'r') as f:
        pred_list = f.readlines()
    for i, (gt, pred) in enumerate(zip(gt_list, pred_list)):
        gold, db_name = gt.split('\t')
        score = evaluate(
            gold=gold, 
            predict=pred, 
            db_name=db_name.strip(),
            db_dir='/local2/jiang719/databases/spider/database/', 
            kmaps=kmaps
        )
        print(i, score)
